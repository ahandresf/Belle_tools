        ...
        if l_out.find("+:")!=-1:
            ...
            if l_out.find(":") == -1: #lines without semicolon
                ...
                if l_out.find("#@@")!=-1:
                    ...
                    l_out=''
                    #l_out='"comment":"%s"' % (l_out)#### NOT GOOD
            else:
                l_out=l_out.replace(" ","")
                l_out=l_out.split(':',1)
    ...
    while(flag==True):
        ...
        #(NO,NO)
            ...
            if next_line.find("}")!=1: #if the next line is } we never put a commaself.
                ...
                if current_line.find("}")!=-1 and next_line.find("}")==-1: # If I find } and next is not } put a comma
                    l_out_current='%s,\n' % (current_line) #put a comma if the next one is a string
                else:
                    if next_line.find("}")!=-1:
                        l_out_current='%s\n' % (current_line)
                        print("suponemos } and whateverafter")
                        print("current",current_line)
                        print("next",next_line)
                        print("linea es",l_out_current)
                        #l_out_current='%s' % (current_line)
                        #l_out_current='%s\n' % (current_line[:-1]) #verify why the spaces think it works properly
                    else:
                        l_out_current='%s\n' % (current_line)
                        #l_out_current='%s\n' % (current_line) ###
